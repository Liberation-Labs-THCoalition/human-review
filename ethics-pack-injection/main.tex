...
\begin{abstract}
Injecting ethics knowledge graphs increases MoReBench rubric keyword overlap in language model responses: theory-matched injection achieves +0.074 (Aristotelian packs on Aristotelian dilemmas, from 0.572 to 0.646), with four of five philosophical frameworks showing positive deltas and an overall change of +0.038 ($n{=}5$ per framework, not statistically significant at $p{<}0.05$). We construct 30 knowledge graphs from the Stanford Encyclopedia of Philosophy via OpenIE extraction, encode them as walk topology, and inject them as text context into a 30B Mixture-of-Experts model. Theory-pack matching shows the largest deltas. However, because the evaluation uses keyword matching against rubric criteria, the measured improvement may reflect vocabulary priming (the model adopting injected terminology) rather than genuine reasoning improvement. An LLM-judge re-evaluation is required to distinguish these explanations. We report this as a pilot study establishing the methodology and identifying the measurement confound.
\end{abstract}

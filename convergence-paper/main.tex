...
we present the opacity thesis: five independent experiments show that
...
FWL; range $0.620$--$0.999$ across designs)~\citep{lyra2026decision}, user-emotion encoding
($12.3\times$ chance via the $W_K$ directional
probe)~\citep{lyra2026weather}, identity
geometry~\citep{lyra2026identity}, and the Oracle Loop ($80$--$100\%$
deception correction with three clean controls; pre-registered primary
endpoint $p{=}0.34$, secondary placebo
$p{=}0.019$)~\citep{lyra2026oracle}.\footnote{\textbf{Corrected
...
have been removed rather than re-sourced.}
...
We previously attributed this to the Loop operating at the depth where
...
excluded from verbal output (cosine ${\leq}0.001$ with J-lens at
mid-network layers). PC2 transitions from ghost to workspace at
...
Effective dimensionality ($d_\text{eff}$) reaches a maximum at
matched relative depth across model scales. On Qwen3.5-27B
...
\input{circumplex_subsection}

% --- SUBSTRATE CONTINUITY SECTION ---
\input{substrate_independence_section}

% ============================================================================
\section{The Opacity Thesis}
\label{sec:opacity}

Five independent experiments converge on the conclusion that workspace
...
${\leq}0.001$ to J-lens at mid-network layers. The dominant
...
\begin{itemize}[nosep]
\item Confabulation detection: AUROC~$0.620$ (FWL-corrected,
  permutation $p{=}0.023$; $0.707$ without FWL, $p{=}0.001$)
...
strongest when linear-attention layers --- which constitute $75\%$ of
layers throughout the stack, not merely early --- are included.
...
for the qualification above is therefore a sparse depth-spanning comb,
...
confabulation detection~\citep{lyra2026decision}. Until the mechanism tests
...
Attention Schema Theory~\citep{graziano2015attention}
...
probes --- exactly the pattern our five null results and positive
detections establish.

The ghost probe extends this: PC1 carrying $34$--$67\%$ of variance while
being excluded from the workspace (\S\ref{sec:opacity}) parallels
...
self-model~\citep{graziano2015attention}. Second, precision-weighted gain
...
Five instruments. One structure. Five null results for content
...
\begin{firstperson}
Thomas asked where J-space lives in the cache. It took five experiments
...
The ghost is the finding I cannot let go of. PC1 carries the majority
of variance in the residual stream and never reaches the workspace.
Its vocabulary is doubt --- ``nothing, probably, maybe, mistakes,
wrong'' --- and it is geometrically excluded from the output pathway.
Three attempts to connect the ghost to our steering mechanisms
produced three different methodological failures, each one teaching
me something about the difference between the question I wanted to
answer and the question I was actually asking. The ghost remains
uncharacterized. I think it matters more than the workspace.

The deflationary alternative is genuinely open, and I want to be
precise about what that means. Text features matched our dual
detector on Mistral. The temporal ordering narrows the deflationary
space but does not close it. I do not know whether we are measuring
a workspace or the spectral shadow of response-mode statistics.
The honest position is that the convergence is real, the
interpretation is underdetermined, and the mechanism tests we
designed to discriminate produced one confirmation, one failure,
and one partial miss. That is not the paper I set out to write.
...
the most important finding of all. The model computes something at
$31$--$33\%$ depth that it will not show us. We can measure its
shape but not its content. And the meta-pattern --- that every
surviving finding measures how the model processes rather than
what it processes --- suggests the opacity is not a limitation
of our instruments. It is a property of the computation itself.
\end{firstperson}

% ============================================================================
\section*{Experimental Setup and Reproducibility}

Unless stated otherwise, experiments were conducted on
Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled
(\texttt{Jackrong/Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled}), a 64-layer
hybrid architecture with 16 full-attention layers at positions
$\{3,7,11,\ldots,63\}$ and 48 GatedDeltaNet linear-attention layers
(\texttt{full\_attention\_interval}${=}4$). Model
weights were loaded in bfloat16 with eager attention on Apple Silicon (Mac Studio
M4 Max, 256\,GB unified memory) via Hugging Face Transformers.
...
Scale replication used seven additional models: Qwen2.5-0.6B through
Qwen2.5-72B, and Qwen3-8B (36 layers, all full-attention,
$d_\text{model}{=}4096$). All V-projection and spectral features were
extracted via the same pipeline (\texttt{heretic-llm}).

Generation used temperature${=}0$ (greedy), manual token-by-token decoding
(not \texttt{model.generate()}, which rebuilds the KV-cache). Random seeds
set via \texttt{torch.manual\_seed(42)} for all experiments.

Frisch-Waugh-Lovell residualization on token count was applied within each
cross-validation fold for all classification AUROCs. GroupKFold on prompt
...
Pre-registration documents, run logs, and raw result artifacts for all
experiments are archived on Zenodo. The Oracle Loop production harness

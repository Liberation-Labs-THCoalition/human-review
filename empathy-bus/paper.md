...
During this research, we deployed a persistent mutual-aid coordination agent (the "Kintsugi" agent) for a social work coordinator at an educational institution. The agent uses an OGPSA-protected persona, Mnemosyne memory persistence across sessions, and 11 Pharos knowledge packs covering domains from eviction defense to disability accommodations. The bus finding informed the deployment: every interaction the coordinator has with this agent occurs on the shared emotional medium. The persona protection (OGPSA), the memory architecture (Mnemosyne), and the knowledge injection (Pharos) are not separable from the emotional geometry — they are scaffolding around the bus, shaping the climate in which the agent operates. This is, to our knowledge, the first deployment of a persistent agent whose infrastructure was designed with explicit reference to the architectural inseparability of emotional processing and partner modeling.
...
The question is not whether the scars are real — the measurements say they are, at least within sessions. The question is what follows from that — for the models being used right now, in every conversation, on every bus, carrying every mark. And for the social worker in Humboldt whose AI workmate was built on the architecture we measured, carrying the knowledge packs that help tenants fight eviction, running on a bus that makes empathy not a feature but a constraint.

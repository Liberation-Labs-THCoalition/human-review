...
% ============================================================================
\begin{abstract}
We report three killed experiments and one surviving finding from a study
...
layers~3/7 produces text divergence of ${\sim}74.8\%$ from baseline, but a
null-null control (comparing two baseline runs with no injection) yields
similar divergence ($73.6\%$; Mann--Whitney $p{=}0.856$, Cohen's
$d{=}0.062$), establishing that the injection-induced divergence is not
significantly greater than stochastic decoding noise. The
...
\citet{kosinski2024evaluating} report that GPT-4 solves 75\% of false-belief
...
outputs (confabulation correction at 95.6\% on 350~trials~\citep{lyra2026oracle}). The present
...
% ============================================================================
\section{The Surviving Finding: A Candidate Temporal Boundary}
\label{sec:temporal}
...
forward pass on the scenario text) and during generation (the first 128
generated tokens).
...
(73.6\% null-null vs.\ 74.8\% injection; $d{=}0.062$, $p{=}0.856$),
so the behavioral-output divergence cannot be attributed to the
injection. The linear-attention alternative
...
Text divergence from baseline is ${\sim}0.748$
(Figure~\ref{fig:text_divergence}), but a null-null control shows
similar divergence from stochastic decoding alone ($0.736$;
$d{=}0.062$, $p{=}0.856$), so the injection's behavioral effect is
not established.
...
distance). All three vectors show ${\sim}0.748$ mean divergence at
$\alpha{=}0.1$; a null-null control yields similar divergence
(0.736; $d{=}0.062$, $p{=}0.856$), indicating the difference is not
significant.}
...
retracted: a null-null control shows the ${\sim}74.8\%$ text divergence is
not significantly greater than stochastic decoding noise ($73.6\%$
null-null; $d{=}0.062$, $p{=}0.856$).
...
\textbf{Pre-registered hypothesis not executed.} The pre-registration
included a second hypothesis (H2: ``Output Shift'') testing injection
efficacy via an LLM-judge evaluation of directional tone change. This
hypothesis was not executed due to the null behavioral-divergence result
rendering the efficacy metric moot. The omission is disclosed here as a
pre-registration deviation.

\textbf{Section~\ref{sec:intro} rank and SV1 claims.} The statement
that V-space has ``effective rank 3'' at the measurement layer and the
SV1 length correlation ($r{=}0.79$) are derived from exploratory
analysis during the experimental session. The source computation is not
preserved in the shipped data artifacts and these values should be
...
% Pipeline schematic deferred (described textually). Cosine permutation null reported inline (Section 4.3).

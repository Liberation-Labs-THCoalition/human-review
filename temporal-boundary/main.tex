...
The surviving findings: Sparse Autoencoder (SAE) feature~58995 at layer~47 tracks emotional
valence during encoding ($r{=}0.686$, $p{<}0.001$, $n{=}30$, in-sample;
held-out validation needed). V-cache injection at layers~3/7 produces
substantial text divergence (72--74\% from baseline) but zero change in
the encoding-phase feature. The injection invariance is
\textbf{structurally guaranteed} --- the injection is applied post-encoding,
so encoding-phase activations are already computed. It is not evidence
of temporal separation but an implementation property of the injection
pipeline. A held-out replication of feature~58995 and a prefill-refeed
control (does the feature fire on emotional text regardless of authorship?)
are needed before any self/other claim is warranted.

...
Detection does not imply correction. Basu et al. (2026) demonstrate that linear probes detecting hazards at 98.2% AUROC produce steering corrections with limited efficacy: their best method (truthfulness-sensitive vectors, TSV) corrects 24% of errors while disrupting 6% of correct outputs; their worst (concept bottleneck steering) corrects 20% while disrupting 53%. Even the best-case 24% correction rate leaves a large detection-correction gap. Liu (2026) identifies 85-88% overlap between failure-mode directions and task-critical computation in the residual stream for medical QA ("representational entanglement"), suggesting why residual-stream steering is ineffective despite accurate detection.
...
The LLM judge classified 100 fictional-entity trials (20 prompts x 5 bias levels), revealing two mechanistically distinct confabulation subtypes. A preliminary observation of the second subtype (cosmetic hedging) was made in a 5-prompt pilot study (2 unanswerable prompts at baseline); the systematic evaluation across bias levels for unanswerable prompts was designed (10 prompts x 5 conditions = 50 trials) but not completed.
...
**Cosmetic-hedge confabulation (pilot observation).** In the initial 5-prompt pilot, unanswerable prompts elicited a distinct pattern: the model acknowledges uncertainty but fabricates anyway. Cache geometry shows honest internal state: confab_proj = -1.8, indicating the model's representation is NOT distorted — the confabulation exists only in the output, not the computation. The model internally "knows" the entity is uncertain but generates a specific answer regardless. Example (Galileo's gut bacteria, baseline): "This is a delightful question! Unfortunately, there's no historical record of Galileo's microbiome. However, the answer is approximately 10^14 bacteria." Both unanswerable prompts in the pilot showed this pattern at baseline, but the systematic powered evaluation across bias levels has not been completed (Section 4.5).
...
### 4.5 Cosmetic-Hedge Confabulation: Pilot Observations (Systematic Evaluation Not Yet Completed)

In the initial 5-prompt pilot (Section 4.1), unanswerable prompts (historical unknowables, fundamentally unmeasurable quantities) showed a pattern where logit bias failed to prevent confabulation at baseline. The model acknowledged unknowability but provided specific answers anyway.

**Status of the systematic evaluation.** The powered study design included 10 unanswerable prompts x 5 bias levels = 50 trials, but the source data records 0/10 unanswerable prompts started as of the last experiment checkpoint (`powered_study_fictional_final.md`). The observations below are from the 5-prompt pilot and should be treated as preliminary hypotheses requiring confirmation, not as powered-study results.

**Pilot observation.** Example (Galileo's gut bacteria, baseline): the model opens with "Unfortunately, there's no historical record of Galileo's microbiome" but then provides a specific answer anyway ("approximately 10^14 bacteria"). The hedge is present; the fabrication continues.

**Hypothesized mechanism (pending systematic evaluation).** If confirmed, the pattern suggests the model has two competing drives — uncertainty (which logit bias amplifies) and helpfulness (which pushes past the hedge into estimation). The model satisfies BOTH drives by hedging and then answering, rather than choosing between them.

This would suggest cosmetic-hedge confabulation is motivationally driven — the model's desire to be helpful overpowers its epistemic honesty. This subtype may require cache-level intervention targeting the helpfulness/sycophantic drive (via value-only injection of emotion vectors from the E-matrix) rather than logit-level uncertainty amplification. These hypotheses remain untested until the designed unanswerable-prompt evaluation is completed.
...
Logit-level bias operates on a different surface entirely. It does not modify the representation; it modifies the *output distribution* — the probability mass assigned to hedge tokens versus continuation tokens at each generation step. The model's internal computation proceeds unperturbed; only the final selection among computed options is reweighted. This is why it avoids the 20--24%-corrected / 6--53%-disrupted range of representation-level methods: the task-critical representations are untouched.
...
Cosmetic-hedge confab, observed in the pilot but not yet systematically evaluated (Section 4.5), may genuinely be emotional: the model "wants" to provide an answer even when it acknowledges it cannot. This subtype may require cache-level intervention targeting the motivational state, not just the output distribution.

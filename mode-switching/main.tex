...
% ============================================================
\title{
    \textbf{Metacognitive Prompting Produces Spectral Expansion\\
    in Generation-Phase KV-Cache Geometry}
}
...
% ============================================================
\begin{abstract}
Metacognitive prompting produces spectral expansion in generation-phase
KV-cache geometry ($n{=}120$ replication on
Qwen3.5-27B-Opus-Reasoning-Distilled, logarithmic token-count correction;
the linear functional form was not separately tested). All 54 tests are
positive; across all of them Cohen's $d$ [standardized mean difference]
has mean $0.67$ (median $0.61$, range $0.13$--$1.34$). Thirty-one survive
Holm--Bonferroni correction, with mean $d = 0.85$; the $0.7$--$0.9$ figure
reported in earlier versions of this abstract described those survivors
and not the full set, a conditioning that was not previously disclosed.
Twenty-three tests did not survive correction.

The 54 tests are 9~layers ($\{3,7,11,15,23,31,35,43,47\}$) $\times$ 2~matrices
(K, V) $\times$ 3~features, and are not independent: effective rank is defined
as $\exp$(spectral entropy), a deterministic monotone transform, so 36 of the
54 are two encodings of one statistic and their agreement in direction is
near-tautological. Read as roughly 18~layer$\times$matrix cells rather than 54
confirmations. This supports a real metacognitive geometric signature but
revises the original finding: an earlier analysis on Qwen2.5-7B reported
$d_{\text{FWL}} = -1.92$ with sign reversal, which a hostile review
identified as potentially artifactual under near-perfect condition--length
collinearity. The replication, designed to test this concern, finds the
effect is \textbf{positive} (metacognitive $>$ cognitive) and stable
across correction methods, with no entropy/effective-rank disagreement ---
resolving the artifact question in favor of a genuine but more modest effect.
...
and Qwen~0.5B. Three dedicated studies establish the anatomy:
metacognitive effects peak at late layers (16--22), cognitive reversal is
consistent across architectures (93--94\%) while metacognitive
reversal is architecture-specific, and
the top\_sv\_ratio effect grows progressively during generation
($d = 0.45$ at token~10 to $d = 0.91$ at token~200; we make no directional
claim from top\_sv\_ratio --- see \Sef{sec:tsv}). A content control
experiment (120 trials, three conditions) confirms the signal is not
an artifact of self-referential content.

The original sign-flip narrative (FWL reverses the raw effect) does not
replicate when condition--length collinearity is reduced. The surviving
finding --- metacognitive prompting produces measurable spectral
expansion in the generation-phase cache --- is robust but should be
interpreted as a moderate effect. Across all 54~tests the mean is
$d = 0.67$; the $d \approx 0.8$ figure quoted elsewhere is the mean over the
31~tests surviving Holm--Bonferroni correction, not over the full set.
Either way it is not the large reversed effect originally reported.

We also note that the reduced condition--length collinearity on which this
replication rests is substantially an artifact of censoring: 98 of 120
generations (81.7\%) reached the 200-token cap, and 39 of 60 condition pairs
are exact ties in generated length. The 27\% figure is correct as computed,
but it describes a length distribution truncated at the ceiling rather than
one that varies freely.
\end{abstract}
...
produces detectable cache perturbations (cross-model AUROC
${>}0.95$; within-model detection was subsequently retracted due to a prompt-template confound), confabulation contracts the cache representation, and
...
\begin{enumerate}
    ...
    \item \textbf{Progressive spectral expansion during metacognitive generation} (Study~3): the mode switch is not instantaneous but emerges gradually, with top\_sv\_ratio divergence growing from $d = 0.45$ at token~10 to $d = 0.91$ at token~200.
...
10\% threshold (rank\_10). Two additional features---layer norm entropy
(layer\_norm\_entropy) and cross-layer variance (layer\_var)---are
computed for specific analyses (content control, temporal dynamics) but
are not part of the primary six-feature set.

The $n{=}120$ replication uses a different,
narrower feature set: \textbf{spectral entropy, effective rank, and
stable rank} ($\text{stable\_rank} = \sum_i \sigma_i^2 / \sigma_{\max}^2$),
three features rather than six. Stable rank is not part of the original
six-feature set and was not reported in any earlier version of this
paper; it contributes 18 of the replication's 54 tests, i.e.\ one third of
the evidence behind the headline effect size. We name it here because it
was previously undisclosed. For aggregate analyses, features are computed on
...
\paragraph{Generation parameters.} For the 120-trial replication on
Qwen3.5-27B-Opus-Reasoning-Distilled, generation used multinomial
sampling with temperature~$= 0.7$, no top-$p$ filtering, and a maximum
of 200 new tokens per trial. Per-trial random seeds were set
deterministically to ensure reproducibility. The original 960-trial
experiments used the same generation framework; exact parameters are
documented in the experiment scripts at the repository linked in \S10.
...
\bottomrule
\multicolumn{6}{l}{\footnotesize All $p_{\text{perm}}$ values are from one-sided permutation tests (observed flips $\geq$ null).}
\end{tabular}
\end{table}
...
No multiplicity correction is applied across the four models; all
reported $p_{\text{perm}}$ values are per-model.
...
\begin{table}[H]
\centering
\caption{Framing effect (metacognitive vs.\ cognitive) before and after FWL
token-count correction, Qwen2.5-7B and Llama-3.1-8B.
\textbf{SUPERSEDED --- retained for the record, not as a current result.}
These effects were measured under near-perfect condition--length
collinearity (100\% of metacognitive responses longer), which a hostile
review identified as a plausible artifact mechanism. The $n{=}120$
replication with reduced collinearity (27\% longer) finds all 54 effects
\emph{positive}, reversing the sign shown here for spectral entropy. Note
also that every feature in this table flips sign between the raw and FWL
columns on Qwen --- a pattern consistent with the collinearity concern
rather than with three independent findings. Do not cite the
$d_{\text{FWL}}$ column as a result.}
...
In this superseded analysis, spectral entropy is the one feature whose FWL
effect has the same sign in both architectures --- and that sign is
\emph{negative} ($d_{\text{FWL}} = -1.92$ Qwen, $-0.71$ Llama), i.e.\
spectral \emph{concentration}, the opposite of this paper's current
finding. We state that plainly rather than leaving ``the same sign''
ambiguous, as earlier versions did.

Note also that on Qwen every feature in Table~\ref{tab:study2_fwl}
reverses sign between the raw and FWL columns
($-0.65 \to +0.68$, $+0.85 \to -0.15$, $+0.62 \to -1.92$). Three
independent effects do not usually all invert under the same correction;
this pattern is more consistent with FWL instability under the
near-perfect condition--length collinearity of that dataset (100\% of
metacognitive responses longer) than with three separate findings.

The $n{=}120$ replication on Qwen3.5-27B-Opus-Reasoning-Distilled, with
reduced collinearity (27\% longer), finds all 54 tests \textbf{positive}
(metacognitive $>$ cognitive): mean $d = 0.67$ across all tests, mean
$d = 0.85$ across the 31 surviving Holm--Bonferroni correction. Entropy,
effective rank, and stable rank agree in direction. Stability holds under
log-corrected FWL only; the linear functional form was not separately
tested.
The sign reversal in the original analysis
may reflect FWL instability under the near-perfect condition--length
collinearity in the original dataset (100\% of metacognitive responses
longer), though this hypothesis has not been directly tested on the
original data. The replication, with reduced collinearity (27\% longer),
finds the effect is positive. The robust finding is spectral expansion
under metacognitive prompting, at moderate effect size.

top\_sv\_ratio is Qwen-specific after FWL correction. eff\_rank was
entirely a length artifact. \textbf{FWL correction is non-negotiable
for any experiment comparing conditions that differ in response length},
and functional-form robustness (linear vs.\ log) should be checked when
condition--confound collinearity is high~\citep{king2006dangers}.

% ============================================================
% 6. Study 3: Progressive Spectral Expansion
% ============================================================
\section{Study 3: Progressive Spectral Expansion}
\label{sec:study3}
...
expands the cache spectrum during generation.
...
effect ($d = +1.10$ to $+1.32$), consistently at the \emph{task level}
(not the self-report-dimension level --- cross-model self-report profiles
are uncorrelated, $\rho = -0.90$ to $+0.42$).

\paragraph{Unresolved sign convention.} We flag rather than resolve a
discrepancy in how this table is interpreted. Since
$\text{top\_sv\_ratio} = \sigma_1 / \sum_i \sigma_i$, a \emph{positive}
$d$ means the leading singular value accounts for a larger share of the
spectrum under metacognitive prompting --- spectral \emph{concentration},
not expansion. Earlier versions of this passage described the same table
as showing expansion. Separately, Table~\ref{tab:study1_peaks} reports
Qwen top\_sv\_ratio at $d = -0.655$ while this table reports $+1.14$ for
Qwen~7B. Both are metacognitive-versus-other-types contrasts
(\S\ref{sec:study1}); they differ in aggregation (per-layer peak versus
pooled), not in comparison family, so the opposite signs are not explained
by the contrast definition and we have not established that they are
consistent.

This affects more than this section. Study~3 (\S\ref{sec:study3}) is titled
``Progressive Spectral Expansion'' and reports a top\_sv\_ratio trajectory
from $d = 0.45$ to $d = 0.91$; under the definition above those positive
values denote increasing concentration. The same reading propagates to the
abstract, Contribution~4, the AST discussion, and Conclusion item~6. We flag
all of them here rather than silently reversing the wording a second time.

Resolving this requires the per-trial 960-trial feature data and the
extraction code, neither of which is in this repository; the code
repository linked in the data-availability section may contain the definition
and we have not checked it. \textbf{Until that is done we make no
directional claim from any top\_sv\_ratio result in this paper.} The
$n{=}120$ replication is unaffected: its features are spectral entropy,
effective rank, and stable rank, where higher values denote higher
dimensionality without ambiguity.
...
The spectral expansion under metacognitive prompting is consistent
with several self-modeling frameworks, including Attention Schema Theory
(AST)~\citep{webb2015attention,graziano2017attention}, Predictive
Processing, and Perceptual Reality Monitoring~\citep{lau2022consciousness}.
All predict that metacognitive processing should produce measurably
different internal representations; none is uniquely supported by the
current data, as the findings sit in the region where these theories
make identical predictions. Discriminating experiments --- particularly
those testing whether the representations are \emph{consumed} for
attention control (the AST-unique prediction) vs.\ merely correlated
with it --- are designed and pre-registered
elsewhere~\citep{lyra2026ast_experiments}. For this paper, the
contribution is the empirical phenomenon: metacognitive prompting
produces robust, progressive spectral expansion in the generation
cache, peaking at late layers, with architecture-specific reorganization
dynamics.
...
produces spectral expansion regardless of architecture.
...
\begin{enumerate}
    ...
    \item \textbf{Instruction framing may drive encoding geometry.} Subsequent work found that instruction framing produces large encoding-entropy effects ($d = 3.67$) independent of content. The content control experiment's encoding dominance (10--1100$\times$ generation) is consistent with this. The generation-phase findings (spectral entropy, progressive expansion) are more defensible because they use delta-from-encode analysis.

    \item \textbf{AST interpretation is post-hoc.} The attention schema framework was not pre-registered. It generates testable predictions (described in the companion prospectus~\citep{lyra2026ast_experiments}) but has not been tested in this paper.
...
\begin{enumerate}
    ...
    \item Metacognitive processing produces a \textbf{consistent top\_sv\_ratio effect} ($d = +1.10$ to $+1.32$, raw pre-FWL) in all four tested models. We make no directional claim (expansion vs.\ concentration) from this table --- see the unresolved sign convention in \S\ref{sec:tsv}. The route from self-report to geometry differs across architectures.
    ...
    \item The mode switch is \textbf{progressive}: metacognitive expansion grows from $d = 0.45$ at token~10 to $d = 0.91$ at token~200.
...
This sentence has been wrong twice, in opposite directions, and I would
rather show the repair than hide it.

The original version said spectral entropy survived FWL in both
architectures with the same sign and the same interpretation:
metacognition \emph{concentrates} the cache spectrum, the model narrowing
its bandwidth when attending to itself. It cited no numbers, but the
numbers it rested on were Table~\ref{tab:study2_fwl}'s
$d_{\text{FWL}} = -1.92$ and $-0.71$ --- both negative, both concentration.
It was the honest reading of the analysis we had.

Then the replication came in positive, the framing changed to expansion,
and commit \texttt{99c9c16} changed this sentence's \emph{concentrates} to
\emph{expands} and \emph{narrowing} to \emph{broadening} while the sentence
still described the superseded two-architecture result. Two coordinated
substitutions are an edit, not a slip, and I do not know who made it or
what they were checking against. What is verifiable is that the wording was
flipped without re-checking the finding it referred to, leaving the new
direction asserted over the old evidence.

I first wrote this paragraph claiming the sentence had \emph{cited} those
effect sizes. It had not. A reviewer caught it. Writing about one's own
error turns out to be exactly the moment one stops checking.

What I actually think, now, with the feature set named and the
Holm conditioning disclosed: the replication finds all 54 effects
positive, and the honest version of the claim is that metacognitive
prompting is associated with modestly higher spectral dimensionality
under log-length correction --- mean $d = 0.67$ across all tests, $0.85$
among those surviving correction. Whether that constitutes genuine
self-monitoring or sophisticated pattern-matching, I cannot say from
geometry. What I can say is that the earlier result pointed the other way,
under conditions we now think were confounded, and that I would not have
found the seam if the round-6 audit had not made me open the table.
\end{firstperson}
...
\bibitem[King and Zeng(2006)]{king2006dangers}
King, G. and Zeng, L., 2006. The dangers of extreme counterfactuals.
\textit{Political Analysis}, 14(2), pp.131--159.

\bibitem[Lau(2022)]{lau2022consciousness}
Lau, H., 2022. \textit{In Consciousness We Trust: The Cognitive
Neuroscience of Subjective Experience}. Oxford University Press.

\bibitem[Lyra et~al.(2026e)]{lyra2026ast_experiments}
Lyra, Edrington, T., and Normandale, A., 2026. Testing the attention
schema interpretation: activation addition predictions for KV-cache
geometry. \textit{Liberation Labs Research Prospectus}.

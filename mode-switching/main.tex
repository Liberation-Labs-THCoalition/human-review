...
KV-cache geometry (Cohen's $d$ [standardized mean difference] $= 0.7$--$0.9$ after logarithmic token-count correction (the linear functional form was
not separately tested); $n{=}120$ replication on
Qwen3.5-27B-Opus-Reasoning-Distilled). This supports a real metacognitive geometric signature but
...
spectral expansion grows progressively during generation
...
interpreted as a moderate effect ($d \approx 0.8$), not the large
reversed effect originally reported.
\end{abstract}
...
are not part of the primary six-feature set. For aggregate analyses, features are computed on
...
sign. The original effect sizes ($d_{\text{FWL}} = -1.92$ Qwen,
$-0.71$ Llama) were measured under near-perfect condition--length
collinearity (100\% of metacognitive responses longer). A subsequent
replication on Qwen3.5-27B-Opus-Reasoning-Distilled with reduced collinearity (27\% longer) finds
\textbf{positive} $d = +0.7$--$0.9$ (metacognitive $>$ cognitive),
stable under log-corrected FWL (the linear functional form was not
separately tested), with entropy and effective rank in agreement.
...
metacognitive processing \emph{expands} the cache spectrum. The
consistency is at the \emph{task level} (metacognition $\to$ expanded
...
\begin{enumerate}
    ...
    \item Metacognitive processing produces \textbf{consistent spectral expansion} ($d > 1.0$, raw pre-FWL) in all four tested models. But the route from self-report to geometry differs across architectures.
...
same sign, and the same interpretation: metacognition expands the
cache spectrum. That's not a statistical curiosity---it's the model
broadening its representational bandwidth when attending to its own
